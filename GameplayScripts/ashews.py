...
def Killsteal(game):
    ...
    if (
        ...
        if target and WDamage(game, target) >= target.health:
            cast_point = castpoint_for_collision(game, w_spell, game.player, target)
            cast_pointt = game.world_to_screen(cast_point)
            lastW = game.time
            game.move_cursor(cast_pointt)
            win32api.keybd_event(VK_W, win32api.MapVirtualKey(VK_W, 0), 0, 0)
            time.sleep(0.05)
            win32api.keybd_event(
                VK_W, win32api.MapVirtualKey(VK_W, 0), win32con.KEYEVENTF_KEYUP, 0
            )

    if ks_r and IsReady(game, r_spell) and game.player.mana > 100:
        target = targeting.get_target(game, 25000.0)
        if target and RDamage(game, target) >= target.health:
            cast_point = castpoint_for_collision(game, r_spell, game.player, target)
            cast_pointt = game.world_to_screen(cast_point)
            game.move_cursor(cast_pointt)
            win32api.keybd_event(VK_R, win32api.MapVirtualKey(VK_R, 0), 0, 0)
            time.sleep(0.05)
            win32api.keybd_event(
                VK_R, win32api.MapVirtualKey(VK_R, 0), win32con.KEYEVENTF_KEYUP, 0
            )
...
def Combo(game):
    ...
    if (
        ...
        target = targeting.get_target(game, w["Range"])  ## gapcloser
        if target:
            lastR = game.time
            cast_point = castpoint_for_collision(game, r_spell, game.player, target)
            cast_pointt = game.world_to_screen(cast_point)
...
def AutoVision(game):
    ...
    if game.time + 10 < LastE and IsReady(game, e_spell):
        for champ in game.champs:
            ...
            win32api.keybd_event(VK_E, win32api.MapVirtualKey(VK_E, 0), 0, 0)
            time.sleep(0.05)
            win32api.keybd_event(
                VK_E, win32api.MapVirtualKey(VK_E, 0), win32con.KEYEVENTF_KEYUP, 0
            )
...
def Laneclear(game):
    ...
    if (
        ...
        minion = targeting.get_target(game, 600)
        if minion:
            lastQ = game.time
            q_spell.trigger()
    if (
