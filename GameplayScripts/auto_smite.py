...
lview_script_info = {
    "script": "smite+",
    "author": "tefan#0922",
    "description": "Auto smites the jungle mob when under the cursor",
}
...
def lview_update(game, ui):
    ...
    is_smitable = (
        hovered
        and (
            hovered.has_tags(UnitTag.Unit_Monster_Large)
            or hovered.has_tags(UnitTag.Unit_Monster_Epic)
        )
        and hovered.health - smite.value <= 0
    )
    ...
    if show_smitable and is_smitable:
        game.draw_circle_world(
            hovered.pos, hovered.gameplay_radius, 30, 3, Color.YELLOW
        )
